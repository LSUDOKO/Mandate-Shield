...
import type { CatalogProduct, ShoppingAgent } from "@mandate-shield/agent";
 ...
 * audit records.
 *
 * This mirrors the Express server's pipeline deliberately rather than importing
 * it. The MCP server has to run as a standalone stdio process for Claude
 * Desktop, with no Express dependency and no HTTP server in the picture, so the
 * shared thing between the two surfaces is the verification engine itself — not
 * the transport that happens to wrap it.
 *
 * The snapshot is taken exactly once and every later stage reads it, so nothing
 * can re-fetch live state mid-flight. That is the TOCTOU fix in practice.
 */
export class Pipeline {
  ...
  async process(
    instruction: string,
    opts: { transactionId?: string; nonce?: string; catalog?: CatalogProduct[] } = {},
  ): Promise<TransactionRecord> {
    const transactionId = opts.transactionId ?? randomUUID();
    const nonce = opts.nonce ?? randomUUID();
    const now = new Date().toISOString();

    // A caller that already knows which listing the user chose narrows the
    // catalog to it, so the agent cannot quietly draft a different product than
    // the one on screen. Callers that pass nothing get the full catalog and the
    // agent's own selection, exactly as the HTTP pipeline behaves.
    const agent = opts.catalog ? this.deps.agent.withCatalog(opts.catalog) : this.deps.agent;

    const draft = await agent.draftOrder(instruction, { transactionId, nonce });
